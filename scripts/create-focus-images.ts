...
type Focus={crop:[number,number,number,number];label:string;box:[number,number,number,number];markers:Array<[number,number,string]>};
const images:Record<string,Focus>={
...
function svg(width:number,height:number,focus:Focus){const [x,y,w,h]=focus.box.map((value,index)=>index%2?value*height:value*width);const marker=focus.markers.map(([mx,my,text])=>`<circle cx="${mx*width}" cy="${my*height}" r="${Math.max(18,width*.04)}" fill="#e8f35f" stroke="#17382d" stroke-width="4"/><text x="${mx*width}" y="${my*height+7}" text-anchor="middle" font-family="Arial" font-size="${Math.max(18,width*.045)}" font-weight="700" fill="#17382d">${text}</text>`).join('');return `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><rect x="${x}" y="${y}" width="${w}" height="${h}" rx="12" fill="#e8f35f" fill-opacity=".15" stroke="#d8ed45" stroke-width="5"/>${marker}<rect x="12" y="12" width="${Math.min(width-24,Math.max(180,focus.label.length*16))}" height="32" rx="8" fill="#17382d"/><text x="24" y="34" font-family="Arial" font-size="17" font-weight="700" fill="#fff">${esc(focus.label)}</text></svg>`;}
await mkdir('public/annotated/github',{recursive:true});
for(const [file,focus] of Object.entries(images)){const source=`private-captures/github/${file}`;if(!existsSync(source)){console.error(`Missing private source: ${source}. Public assets are never used as capture sources.`);process.exit(1);}const meta=await sharp(source).metadata();if(!meta.width||!meta.height)continue;const [x,y,w,h]=focus.crop;const width=Math.round(meta.width*w),height=Math.round(meta.height*h);const crop=sharp(source).extract({left:Math.round(meta.width*x),top:Math.round(meta.height*y),width,height});await crop.composite([{input:Buffer.from(svg(width,height,focus)),top:0,left:0}]).png().toFile(`public/annotated/github/focus-${file.replace('.png','')}.png`);console.log(`focused ${file}`);}
