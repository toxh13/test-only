...
for(const [file,focus] of Object.entries(images)){const source=`public/screenshots/github/${file}`;const meta=await sharp(source).metadata();if(!meta.width||!meta.height)continue;const [x,y,w,h]=focus.crop;const width=Math.round(meta.width*w),height=Math.round(meta.height*h);const crop=sharp(source).extract({left:Math.round(meta.width*x),top:Math.round(meta.height*y),width,height});await crop.composite([{input:Buffer.from(svg(width,height,focus)),top:0,left:0}]).png().toFile(`public/annotated/github/focus-${file.replace('.png','')}.png`);console.log(`focused ${file}`);}
